...
class RiskPolicy:
    ...
    def __init__(
        self, tiers: Optional[List[RiskTier]] = None, policy_version: str = "v1.0"
    ):
        """
    ...
    def calculate_max_amount(
        self, monthly_income: float, dti_ratio: float, debt_ratio: float
    ) -> Optional[float]:
        """
    ...
    def evaluate(
        ...
        max_amount = None
        if monthly_income > 0:
            max_amount = self.calculate_max_amount(
                monthly_income, debt_ratio, debt_ratio
            )

            # Check if requested amount exceeds recommendation
            if requested_amount and requested_amount > max_amount:
                conditions.append(
                    f"Requested amount exceeds recommendation ({max_amount:,.0f})"
                )
