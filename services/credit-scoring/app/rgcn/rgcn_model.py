...
class RGCNCreditModel(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index_dict: dict, edge_type_dict: dict) -> torch.Tensor:
    ...
    def _build_edge_type(self, edge_index_dict: dict, edge_type_dict: dict) -> torch.Tensor:
        all_types = []
    ...
    def get_embeddings(self, x: torch.Tensor, edge_index_dict: dict) -> torch.Tensor:
        self.eval()
        with torch.no_grad():
            hetero_edge_index = self._build_hetero_edge_index(edge_index_dict)
...
def get_relation_keys(data: HeteroData) -> list:
    return [et for et in data.edge_types if et[0] == et[2]]