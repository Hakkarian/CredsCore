...
class RGCNFeatureFactory:
    ...
    def initialize(self, model: RGCNCreditModel, graph_builder: GraphBuilder, graph_data: HeteroData):
        self.model = model
    ...
    def extract_features(self, applicant_data: pd.DataFrame) -> Tuple[np.ndarray, list]:
    ...
    def _prepare_features(self, applicant_data: pd.DataFrame) -> pd.DataFrame:
        if self.graph_builder.scaler is not None:
            if isinstance(applicant_data, pd.DataFrame):
                feature_cols = [c for c in applicant_data.columns if c in self.feature_names]
    ...
    def save(self, save_dir: str):
        ...
        torch.save(self.model.state_dict(), os.path.join(save_dir, "rgcn_model.pt"))
        joblib.dump({
